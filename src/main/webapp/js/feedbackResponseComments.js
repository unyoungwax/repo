...
function registerResponseCommentsEvent() {
    ...
    //$('[id^=showResponseCommentAddForm]').each();
}

function loadResponseCommentAddFormByAjax(recipientIndex, giverIndex, qnIndx) {
    var formContainerId = 'showResponseCommentAddForm' + '-' + recipientIndex + '-' + giverIndex + '-' + qnIndx;
    var $addResponseCommentFormContainer = $('#' + formContainerId);
    // the variable form refers to the form containing the data for retrieving the Add Response Comment form
    // not the Add Response Comment Form itself
    var $form = $addResponseCommentFormContainer.find('form'); 
    ...
    $.ajax({
        ...
        beforeSend: function() {
            $addResponseCommentFormContainer.html("<img src='/images/ajax-loader.gif'/>");
        },
        success: function(data) {
            $addResponseCommentFormContainer.html(data);
            $addResponseCommentFormContainer.find("a[id^='button_save_comment_for_add']")
...
function showResponseCommentAddForm(recipientIndex, giverIndex, qnIndx) {
    loadResponseCommentAddFormByAjax(recipientIndex, giverIndex, qnIndx);
